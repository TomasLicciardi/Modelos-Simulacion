...
class Local:
    ...
    def simular(self):
        ...
        while tiempo_actual < self.tiempo_fin_operacion:
            if 0 <= tiempo_actual < 1800 or 12600 <= tiempo_actual < 14400:
                if random.random() < 1/250:
                    cliente = Cliente(tiempo_actual)
                    self.cola.append(cliente)
            elif 1800 <= tiempo_actual < 3600 or 10800 <= tiempo_actual < 12600:
                if random.random() < 1/210:
                    cliente = Cliente(tiempo_actual)
                    self.cola.append(cliente)
            elif 3600 <= tiempo_actual < 5400 or 9000 <= tiempo_actual < 10800:
                if random.random() < 1/130:
                    cliente = Cliente(tiempo_actual)
                    self.cola.append(cliente)
            elif 5400 <= tiempo_actual < 9000:
    ...
    def actualizar_pantalla(self, tiempo_actual):
        with screen_lock:
            ...
            # Mostrar "TP7. Tomás Licciardi" en la parte superior
            font_titulo = pygame.font.SysFont(None, 50)
            titulo = font_titulo.render("TP8. Tomás Licciardi", True, (255, 255, 255))
            screen.blit(titulo, (50, 20))

            # Hora actual en formato HH:MM desde las 8:00 AM
            hora_actual = f"{8 + tiempo_actual // 3600:02.0f}:{(tiempo_actual % 3600) // 60:02.0f}"
            font_hora = pygame.font.SysFont(None, 30)
            texto_hora = font_hora.render(f"Hora: {hora_actual}", True, (255, 255, 255))
            screen.blit(texto_hora, (50, 70))

            x_boxes = 900
            y_boxes = 50
            for i, box in enumerate(self.boxes):
                color = (255, 0, 0) if box.ocupado else (0, 255, 0)
                pygame.draw.rect(screen, color, (x_boxes, y_boxes + i * 100, 80, 80), 15)
...
if __name__ == "__main__":
    ...
    nombre_archivo = "animacion_4_boxes.avi"
    contador = 2
